# Development

- [Claude Code](./ClaudeCode/README.md)

## AI Agent Developer Specialization

- Specialization by Vanderbilt Uni [AI Agent Developer Specialization](https://www.coursera.org/specializations/ai-agents)
- Beginner level

1. [AI Agents and Agentic AI with Python & Generative AI](./AiAgent_WithPythonAndGenerativeAI.md)
2. [AI Agents and Agentic AI Architecture in Python](./AiAgent_ArchitectureInPython.md)
3. [OpenAI GPTs: Creating your own custom AI Assistants](./AiAgent_OpenAiGPT_CreateCustomAiAssistants.md)
4. [Prompt Engineering for ChatGPT](./AiAgent_PromptEngineeringForChatGPT.md)
5. [ChatGPT Advanced Data Analysis](./AiAgent_ChatGPT_AdvancedDataAnalysis.md)
6. [Trustworthy Generative AI](./AiAgent_TrustworthyGenerativeAI.md)

## AI-Assisted Coding

1. [Introduction to AI-assisted Coding](./AiAssistedCoding_Intro.md)
2. [Advanced AI-assisted Development](./AiAssistedCoding_Advanced.md)

## IBM RAG and Agentic AI Professional Certificate

- Coursera from IBM <https://www.coursera.org/professional-certificates/ibm-rag-and-agentic-ai>
- Advanced level

## Misc

  1. [Model Context Protocol](./ModelContextProtocol.md)
  1. [Agents & Chatbots with LangGraph](./AgentsAndChatbotsWithLangGraph.md)
